class LazyjiraAT041 < Formula
  desc "Terminal UI for Jira boards and tickets (pinned v0.4.1)"
  ...
  if OS.mac? && Hardware::CPU.arm?
    ...
    odie "lazyjira@0.4.1: unsupported platform (macOS Apple Silicon or Intel, or Linux x86_64 only)"
  end
  ...
  def install
    ...
    # Distinct name so `lazyjira` (latest) and this formula can both be linked.
    bin.install binary => "lazyjira-0.4.1"
  end

  test do
    assert_predicate bin/"lazyjira-0.4.1", :executable?
  end
end
